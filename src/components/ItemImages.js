...
import { getFullStorageItemPath3, listFilesAndDirectoriesGeneric } from '../functions/firebase-storage-functions'
      ...
      if(listData){
        ...
        for (const subitem of listData){
          filename = subitem.path.substring(subitem.path.lastIndexOf('/') ) // + 1 ????
           getFullStorageItemPath3(collection, itemId, filename, imagesUrlsArray, setIsLoading )
        }
      } 
